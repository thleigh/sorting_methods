...
# the second part will marge two smaller lists into a single larger list
    # I'l ahave the iterate through each item in the two lists and compare them all in a WORST CASE SCENARIO
...
# output will be the single list but sroted from LEAST to GREATEST
# is it okay if I alter the origina list? or do I need to return a completely new list?
# EITHER IS ACCEPTABLE
# What happens if there is only one or zero items?
# If there are one or zero items, just return the list
# should I make htis into a single function for more practical use such as exporting it?
...
def merge_sort(lst):
    def split(ls):
        ...
        # return merge() call in some fastion
        if len(ls) <= 1:
            return ls
        # // will ensure that we haeve an integer rather than float numbers
        midpoint = len(ls) // 2
...
def merge(ls1, ls2):
    ...
    while len(ls1) and len(ls2):
        ...
        # so I was asking myself how do I compare an empy list with a non-emoy list?
